...
function result(students, groups) {
  // your code here
  const sortByFirstName = students?.sort((a, b) => {
    return a.firstName.localeCompare(b.firstName);
  })

  const perGroup = Math.ceil(sortByFirstName?.length / groups)

  return new Array(groups).fill('').map((_, i) => sortByFirstName?.slice(i * perGroup, (i + 1) * perGroup))
}
